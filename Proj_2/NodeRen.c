...
void sendFile(struct seDe *CurrState)
{
  ...
  printf("From Client: %s", MessBuff2);
...
//   PeerAddr.sin_addr.s_addr = inet_addr(SourceIP);
//   PeerAddr.sin_port = htons(sPort);
//   server_len = sizeof(PeerAddr);
//   if((RC = connect(uPortSocket, (struct sockaddr *)&PeerAddr, server_len)) == 1)
//   {
//      return -4;
//   }
...
//   printf("-----Connecting to: %s:%d\n",SourceIP,sPort);
  ...
  //write(PeerSocket, OutBuffer, strlen(OutBuffer));
...
//   printf("-----New Peer Port STR: %s\n",HandlerPort);
  ...
  */
  while(MoreSegs > 0)
  {
    ...
    sendMessage(SourceIP, HandlerPort, "ACK");
    if((TempSoc = accept(uPortSocket, (struct sockaddr *) &uPortSocket, &SockSize)) < 0)
    {
      //Problem with socket connections
      //continue;
      printf("-----%s Unexpected socket errors, QHandler thread has stopped.\n", NodeName);
    ...
    MesgLen = read(TempSoc, InBuffer, 1000);
    InBuffer[MesgLen] = '\0';
    //printf("-----From peer, len: %d \n%s\n\n", MesgLen, InBuffer);
    j ++;
    if(InBuffer[0] == 'E')
    {
